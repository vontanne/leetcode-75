...
func PredictPartyVictory(senate string) string {
	...
	for len(radiant) > 0 && len(dire) > 0 {
		...
		if radiantIndex < direIndex {
			radiant = append(radiant, radiantIndex+n)
		} else {
			dire = append(dire, direIndex+n)
		}
	}
...
/*
Time Complexity: O(n)
	- Initial queue building: O(n)
	- Each senator is processed at most twice (banned or wins)
	- Total iterations bounded by 2n

Space Complexity: O(n)
	- Two queues store indices of all senators
*/
