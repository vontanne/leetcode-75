...
Example 4:
	...
	going left. On the other side, the asteroid 2 makes the asteroid -1 explode and then
	continues going right, without reaching asteroid 4.
...
func AsteroidCollision(asteroids []int) []int {
	...
	for _, asteroid := range asteroids {
		...
		for survived && top >= 0 && asteroids[top] > 0 && asteroid < 0 {
...
Time Complexity: O(n)
	...
	- Inner loop total iterations across all elements is bounded by n

Space Complexity: O(1)
	- In-place modification using input slice as stack
	- Only using constant number of variables (top, survived)
*/
